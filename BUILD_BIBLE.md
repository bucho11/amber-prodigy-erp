...
- Next candidates: payments/POS (Stripe Connect) → clinical docs (HIPAA-grade) → accounting/GL ledger (posting engine; system of record) → marketing (Twilio/A2P) → inventory → staff ops + hours (lets Amber add real bookable providers — currently only Amber) → embedded payroll (Gusto/Check) → reporting → AI layer.
