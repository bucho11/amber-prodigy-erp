...
- Next candidates: scheduling/calendar engine → auto-protocol scheduler (the wedge) → payments/POS (Stripe Connect) + clinical docs (HIPAA-grade) → accounting/GL ledger (posting engine; system of record) → marketing (Twilio/A2P) → inventory → staff ops + hours → embedded payroll (Gusto/Check) → reporting → AI layer.
