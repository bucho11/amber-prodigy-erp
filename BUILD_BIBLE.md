...
- Next candidates: payments/POS (Stripe Connect — needs Amber's connected Stripe account) → clinical docs (HIPAA-grade) → accounting/GL ledger (posting engine; system of record) → provider working hours / availability (open-slot booking) → marketing (Twilio/A2P) → inventory → embedded payroll (Gusto/Check) → reporting → AI layer.
